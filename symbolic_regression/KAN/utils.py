...
SYMBOLIC_LIB = {'x': (lambda x: x, lambda x: x),
                 ...
                 'x^6': (lambda x: x**6, lambda x: x**6),
                 'y': (lambda x: x, lambda x: x),
                    'y^2': (lambda x: x**2, lambda x: x**2),
                    'y^3': (lambda x: x**3, lambda x: x**3),
                    'y^4': (lambda x: x**4, lambda x: x**4),
                ...
                #  'sin(x^2)': (lambda x: torch.sin(x**2), lambda x: sympy.sin(x**2)),
...
def evaluate_complexity(expr):
    ...
    # Transformations for parsing string expressions
    transformations = (standard_transformations + (implicit_multiplication_application, convert_xor))

    # Convert _Program object to a sympy expression if necessary
    if hasattr(expr, '__str__'):
        expr_str = str(expr)
        expr = parse_expr(expr_str, transformations=transformations, local_dict={'add': add, 'mul': mul, 'div': div, 'sub': sub})

    # Recursive function to evaluate the complexity
    def complexity(expr):
        ...
        if isinstance(expr, (sp.Integer, sp.Float, sp.Rational)):
            ...
            complexity_score += 1
        elif expr.func in scoring:
            complexity_score += scoring[expr.func]
            for arg in expr.args:
                complexity(arg)
        elif isinstance(expr, sp.Mul):
            if any(isinstance(arg, sp.Pow) and arg.args[1] == -1 for arg in expr.args):
                complexity_score += 2  # Special handling for division
            else:
                complexity_score += 1
            for arg in expr.args:
                complexity(arg)
