...
class KANLayer(nn.Module):
    ...
    def __init__(self, in_dim=3, out_dim=2, num=5, k=3, noise_scale=0.1, scale_base=1.0, scale_sp=1.0, base_fun=torch.nn.SiLU(), grid_eps=0.02, grid_range=[-1, 1], sp_trainable=True, sb_trainable=True, device='cpu'):
        ...
        noises = (torch.rand(size, self.grid.shape[1]) - 0.5) * noise_scale / num
    ...
    def update_grid_from_samples(self, x):
        ...
        self.grid.data.copy_(self.grid_eps * grid_uniform + (1 - self.grid_eps) * grid_adaptive)
        self.coef.data.copy_(curve2coef(x_pos, y_eval, self.grid, self.k, device=self.device))
    ...
    def initialize_grid_from_parent(self, parent, x):
        ...
        sp2.coef.data.copy_(curve2coef(sp2.grid, x_pos, sp2.grid, k=1, device=self.device))
        y_eval = coef2curve(x_eval, parent.grid, parent.coef, parent.k, device=self.device)
        percentile = torch.linspace(-1, 1, self.num + 1).to(self.device)
        self.grid.data.copy_(sp2(percentile.unsqueeze(dim=1))[0].permute(1, 0))
        self.coef.data.copy_(curve2coef(x_eval, y_eval, self.grid, self.k, self.device))

    def get_subset(self, in_id, out_id):
        spb = KANLayer(len(in_id), len(out_id), self.num, self.k, base_fun=self.base_fun, device=self.device)
        spb.grid.data.copy_(self.grid.reshape(self.out_dim, self.in_dim, spb.num + 1)[out_id][:, in_id].reshape(-1, spb.num + 1))
        spb.coef.data.copy_(self.coef.reshape(self.out_dim, self.in_dim, spb.coef.shape[1])[out_id][:, in_id].reshape(-1, spb.coef.shape[1]))
        spb.scale_base.data.copy_(self.scale_base.reshape(self.out_dim, self.in_dim)[out_id][:, in_id].reshape(-1, ))
        spb.scale_sp.data.copy_(self.scale_sp.reshape(self.out_dim, self.in_dim)[out_id][:, in_id].reshape(-1, ))
        spb.mask.data.copy_(self.mask.reshape(self.out_dim, self.in_dim)[out_id][:, in_id].reshape(-1, ))
    ...
    def unlock(self, ids):
        ...
        locked = True
        for i in range(num):
            locked = locked and (self.weight_sharing[ids[i][1] * self.in_dim + ids[i][0]] == self.weight_sharing[ids[0][1] * self.in_dim + ids[0][0]])
        if not locked:
