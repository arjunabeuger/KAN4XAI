...
class SymbolicNet(nn.Module):
    ...
    def __init__(self, symbolic_depth, funcs=None, initial_weights=None, init_stddev=0.1, mlp_hidden_dim=128):
        ...
        self.mlp = nn.Sequential(
            nn.Linear(len(funcs), self.mlp_hidden_dim),
            nn.ReLU(),
...
class SymbolicNetL0(nn.Module):
    ...
    def __init__(self, symbolic_depth, in_dim=1, funcs=None, initial_weights=None, init_stddev=0.1, mlp_hidden_dim=128):
        ...
        #     nn.Linear(self.mlp_hidden_dim, 1)
        # )  # Add an MLP layer
        self.mlp = KAN(width=[len(initial_weights[0]),1, 1], grid=10, k=24)
    ...
    def forward(self, input, sample=True, reuse_u=False):
        ...
        # print(h.shape, len(self.funcs))
        h = self.mlp(h)  # Pass through MLP
        return h

    def get_loss(self):
        return self.mlp.get_loss()
