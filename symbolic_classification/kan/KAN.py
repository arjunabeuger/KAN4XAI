...
class KAN(nn.Module):
    ...
    def train(self, dataset, opt="LBFGS", steps=100, log=1, lamb=0., lamb_l1=1., lamb_entropy=2., lamb_coef=0., lamb_coefdiff=0., update_grid=True, grid_update_num=10, loss_fn=None, lr=1., stop_grid_update_step=50, batch=-1,
        ...
        pbar = tqdm(range(steps), desc='description', ncols=140)
        ...
        scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=0.1, patience=80,)
        results = {}
        results['train_loss'] = []
        results['test_loss'] = []
        results['reg'] = []
        ...
        for _ in pbar:
            ...
            if opt == "Adam":
                ...
                if sglr_avoid == True:
                    ...
                    # print(pred, dataset['train_label'][train_id])
            ...
            if _ % log == 0:
                if metrics != None:
                    pbar.set_description(f"train loss: {torch.sqrt(train_loss).cpu().detach().numpy():.2e} | test loss: {torch.sqrt(test_loss).cpu().detach().numpy():.2e} | train f1: {results['train_f1_score'][-1]:.3f} | test f1: {results['test_f1_score'][-1]:.3f} | lr: {optimizer.param_groups[0]['lr']:.2e} ")
                else:
                    pbar.set_description(f"train loss: {torch.sqrt(train_loss).cpu().detach().numpy():.2e} | test loss: {torch.sqrt(test_loss).cpu().detach().numpy():.2e} | lr: {optimizer.param_groups[0]['lr']:.2e} ")
    ...
    def auto_symbolic(self, a_range=(-1, 1), b_range=(-1, 1), lib=None, verbose=1):
        '''
